...
const SEED_USER_ID = "user_2i6gGiu54WCu9a07Pd5xiQJZthI";
...
const generateTransactionsForDay = (day: Date) => {
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    const isExpense = Math.random() > 0.6; // 60% change of being a expense
    const amount = generateRandomAmount(category);
    const formttedAmount = convertAmountToMiliunits(
      isExpense ? -amount : amount
    ); //Nagative for expense
